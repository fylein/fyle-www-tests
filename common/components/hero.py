...
def assert_customer_logo_v2(browser, logo_width):
    logo_container = browser.find(f'//section[contains(@class, "customer-logos-v2")]//div[contains(@class, "container")]', scroll=True)
    ...
    time.sleep(3)
    assert logo and logo.is_displayed(), f'Logo is not displayed'
...
def assert_product_mobile_hero_spacing(browser, img_width, logo_width, section_class='product-hero'):
    #H1 spacing assertion
    heading = browser.find(f'//section[contains(@class, "gradient-background") and contains(@class, "{section_class}")]//h1')
    assert_spacing('bottom', heading, 16)
